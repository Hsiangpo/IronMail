...
发送过程中每成功一封邮件，程序都会把进度写入 `logs/progress/`。如果网络中断、SMTP失败或程序退出，下次在 `发送邮件` 里选择同一个收件名单和同一个模板并点击 `开始发送` 时，会弹出断点提示：

- `是`: 跳过已经成功发送的行，只继续未完成部分
- `否`: 清空旧进度，从第一行重新发送
- `取消`: 不发送，返回

如果这组名单上次已经全部发送完成，会提示是否重新发送，选择 `否` 即可避免重复发邮件。
