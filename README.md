...
这是邮件批量发送工具的工程化版本，不包含原来的 GUI 授权壳。
...
- `Mails/收件人名单/`: 收件人表格目录，所有 `.xlsx` / `.csv` 名单都放这里
...
启动后会进入 IronMail 控制台，常用功能都可以在终端里完成：

- `开始发送邮件`: 选择收件人名单，再选择邮件模板后开始发送
- `管理发件邮箱`: 查看、新增、修改、删除发件邮箱，并测试 SMTP 登录
- `调整发送设置`: 修改发送间隔、失败重试次数、每个邮箱连续发送几封后切换
- `设置授权码`: 保存客户端授权码
- `管理邮件模板`: 查看、新增、打开修改、删除 `.md` 邮件模板
...
发送过程中每成功一封邮件，程序都会把进度写入 `logs/progress/`。如果网络中断、SMTP失败或程序退出，下次选择同一个收件人名单和同一个模板时，终端会提示：
...
收件人名单统一放在 `Mails/收件人名单/` 目录。`config/config.yaml` 里的 `excel_file` 留空时，程序会在开始发送前列出这个目录里的 `.xlsx` / `.csv` 文件，让用户选择本次要发送的表格。只有一个表格时会自动使用。

新流程推荐把邮件主题和正文放在 `Mails/邮件模板/` 里的 `.md` 模板文件中。发送前程序会在选择收件人名单后继续选择邮件模板，然后按每一行数据替换模板变量。

收件人名单最低需要 `邮箱` 字段。常用字段如下：
...
变量写法是 `{{字段名}}`，字段名必须和收件人名单的表头一致。更详细的模板教学在 `Mails/邮件模板/README.md`。
...
客户端授权码建议在终端菜单的 `设置授权码` 里填写。配置文件里的结构如下：
...
发件邮箱建议通过程序终端菜单配置，或者在本地配置文件里填写。这个配置文件可能包含邮箱应用专用密码，不要提交到 Git。
