...
- `开始发送邮件`: 选择收件名单，再选择邮件模板后开始发送
- `管理发件邮箱`: 查看、新增、修改、删除发件邮箱，并测试 SMTP 登录
- `管理收件名单`: 打开收件名单文件夹、查看表格、修改表头变量、删除表格
...
发送过程中每成功一封邮件，程序都会把进度写入 `logs/progress/`。如果网络中断、SMTP失败或程序退出，下次选择同一个收件名单和同一个模板时，终端会提示：
...
客户端授权码建议在终端菜单的 `配置` 里填写。配置文件里的结构如下：
